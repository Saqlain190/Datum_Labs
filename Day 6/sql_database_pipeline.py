...
def load_select_tables_from_database() -> None:
    ...
    """
    # Create a pipeline
    pipeline = dlt.pipeline(pipeline_name="rfam", destination='snowflake', dataset_name="rfam_data")
...
def load_standalone_table_resource() -> None:
    ...
    pipeline = dlt.pipeline(
        pipeline_name="pg_database",
        destination='snowflake',
        dataset_name="postgs_data",
        dev_mode=True,
    )
    ...
    orders = sql_table(
        credentials=ConnectionStringCredentials(
            # "mysql+pymysql://rfamro@mysql-rfam-public.ebi.ac.uk:4497/Rfam"
            "postgresql+psycopg2://postgres:neEs2w6QNU0YplJMChzGuxA8Zo7DI93@104.248.248.69:54321/postgres"

        ),
        table="orders",
    )
    # columns will be empty here due to defer_table_reflect set to True
    print(orders.compute_table_schema())

    # Run the resources together
    info = pipeline.run(orders, write_disposition="merge")
    print(info)
